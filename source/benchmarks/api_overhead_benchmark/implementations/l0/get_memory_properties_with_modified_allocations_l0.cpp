...
static TestResult run(const GetMemoryPropertiesWithModifiedAllocationsArguments &arguments, Statistics &statistics) {
    ...
    auto modifyAllocations = [&]() {
        for (int64_t index = 0; index < arguments.AllocationsCount; index++) {
            ...
            if (index & 1) {
                ...
                const ze_device_mem_alloc_desc_t deviceAllocationDesc{ZE_STRUCTURE_TYPE_DEVICE_MEM_ALLOC_DESC};
                void *ptr = nullptr;
                ASSERT_ZE_RESULT_SUCCESS(zeMemAllocDevice(levelzero.context, &deviceAllocationDesc, sizeof(uint64_t), 8u, levelzero.device, &ptr));
...
static RegisterTestCaseImplementation<GetMemoryPropertiesWithModifiedAllocations> registerTestCase(run, Api::L0);
