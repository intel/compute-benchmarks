...
static TestResult run(const UsmCopyRegionArguments &arguments, Statistics &statistics) {
    ...
    const ze_copy_region_t reg = {(uint32_t)arguments.region[0], (uint32_t)arguments.region[1], (uint32_t)arguments.region[2], (uint32_t)arguments.origin[0], (uint32_t)arguments.origin[1], (uint32_t)arguments.origin[2]};
