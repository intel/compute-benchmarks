...
static TestResult run(const SubmitKernelsAsyncArguments &arguments, Statistics &statistics) {
    ...
    for (auto i = 0u; i < arguments.iterations; i++) {
        ...
        for (std::size_t idx = 0; idx < numK; ++idx) {
            ...
            if (arguments.useEventsForSync) {
                ASSERT_ZE_RESULT_SUCCESS(zeEventHostSynchronize(event /* pEvents[idx] */, std::numeric_limits<uint64_t>::max()));
                ASSERT_ZE_RESULT_SUCCESS(zeEventHostReset(event /* pEvents[idx] */));
            } else {
                ASSERT_ZE_RESULT_SUCCESS(zeCommandListHostSynchronize(cmdList, std::numeric_limits<uint64_t>::max()));
            }
        }
...
[[maybe_unused]] static RegisterTestCaseImplementation<SubmitKernelsAsync> registerTestCase(run, Api::L0);